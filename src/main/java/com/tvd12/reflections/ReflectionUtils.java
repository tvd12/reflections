...
public abstract class ReflectionUtils {
    ...
    public static Set<Class<?>> getSuperTypes(
        Class<?> type
    ) {
        ...
        if (superclass != null
            && (includeObject || !superclass.equals(Object.class))
        ) {
    ...
    public static List<Method> getMethodList(
        ...
        return filterToList(t.isInterface()
            ? t.getMethods()
            : t.getDeclaredMethods(), predicates);
    }
    ...
    public static <T extends AnnotatedElement> Set<Annotation> getAnnotations(
        T type,
    ...
    public static <T extends AnnotatedElement> Set<T> getAll(
    ...
    public static <T extends Member> Predicate<T> withName(
        final String name
    ) {
    ...
    public static <T extends Member> Predicate<T> withPrefix(
        final String prefix
    ) {
        return input -> input != null
     ...
     * </pre>
     * */
    public static <T extends AnnotatedElement> Predicate<T> withPattern(
        final String regex
    ) {
    ...
    public static <T extends AnnotatedElement> Predicate<T> withAnnotation(
        ...
        return input -> input != null
            && input.isAnnotationPresent(annotation);
    }
    ...
    public static <T extends AnnotatedElement> Predicate<T> withAnnotations(
        ...
        return input -> input != null
            && Arrays.equals(annotations, annotationTypes(input.getAnnotations()));
    }
    ...
    public static Predicate<Member> withParameters(
        final Class<?>... types
    ) {
    ...
    public static Predicate<Member> withParametersAssignableTo(
        final Class... types
    ) {
    ...
    public static Predicate<Member> withAnyParameterAnnotation(
        final Class<? extends Annotation> annotationClass
    ) {
        return input -> input != null && Iterables.any(
            annotationTypes(parameterAnnotations(input)),
            (Predicate<Class<? extends Annotation>>) input1 ->
                input1.equals(annotationClass)
        );
    }
    ...
    public static Predicate<Member> withAnyParameterAnnotation(
        ...
        return input -> input != null
            && Iterables.any(parameterAnnotations(input),
            (Predicate<Annotation>) input1 ->
                areAnnotationMembersMatching(annotation, input1));
    }

    /** when field type equal given {@code type} */
    public static <T> Predicate<Field> withType(
        final Class<T> type
    ) {
        return input -> input != null
    ...
    public static <T> Predicate<Field> withTypeAssignableTo(
        final Class<T> type
    ) {
        return input -> input != null
            && type.isAssignableFrom(input.getType());
    }

    /** when method return type equal given {@code type} */
    public static <T> Predicate<Method> withReturnType(
        final Class<T> type
    ) {
        return input -> input != null
            && input.getReturnType().equals(type);
    }

    /** when method return type assignable from given {@code type} */
    public static <T> Predicate<Method> withReturnTypeAssignableTo(
        final Class<T> type
    ) {
        return input -> input != null
            && type.isAssignableFrom(input.getReturnType());
    }
     ...
     * </pre>
     */
    public static <T extends Member> Predicate<T> withModifier(
        final int mod
    ) {
        return input -> input != null
            && (input.getModifiers() & mod) != 0;
    }
     ...
     * </pre>
     */
    public static Predicate<Class<?>> withClassModifier(
        final int mod
    ) {
        return input -> input != null
            && (input.getModifiers() & mod) != 0;
     ...
     * <p>if optional {@link ClassLoader}s are not specified, then both {@link com.tvd12.reflections.util.ClasspathHelper#contextClassLoader()} and {@link com.tvd12.reflections.util.ClasspathHelper#staticClassLoader()} are used
     * */
    public static Class<?> forName(
        ...
        if (getPrimitiveNames().contains(typeName)) {
            ...
            List<ReflectionsException> reflectionsExceptions =
                Lists.newArrayList();
                ...
                if (type.contains("[")) {
                    try {
                        ...
                        reflectionsExceptions.add(
                            new ReflectionsException(
                                "could not get type for name " + typeName,
                                e
                            )
                        );
                    }
            ...
            if (Reflections.log != null) {
                for (ReflectionsException reflectionsException : reflectionsExceptions) {
                    Reflections.log.warn(
                        "could not get type for name {} from any class loader",
                        typeName,
    ...
    public static <T> List<Class<? extends T>> forNames(
        final Iterable<String> classes, ClassLoader... classLoaders
    ) {
    ...
    private static Set<Class<? extends Annotation>> annotationTypes(
    ...
    private static Class<? extends Annotation>[] annotationTypes(
        Annotation[] annotations
    ) {
    ...
    //
    private static final List<String> primitiveNames = Lists
        .newArrayList(
            "boolean", "char", "byte", "short",
            "int", "long", "float", "double", "void"
        );
    private static final List<Class> primitiveTypes = Lists
        .newArrayList(
            boolean.class, char.class, byte.class,
            short.class, int.class, long.class,
            float.class, double.class, void.class
        );
    private static final List<String> primitiveDescriptors = Lists
        .newArrayList(
            "Z", "C", "B", "S", "I", "J", "F", "D", "V"
        );
    ...
    private static boolean areAnnotationMembersMatching(
        ...
        if (annotation2 != null
            && annotation1.annotationType() == annotation2.annotationType()
        ) {
            for (Method method : annotation1.annotationType().getDeclaredMethods()) {
                try {
                    if (!method
                        .invoke(annotation1)
                        .equals(method.invoke(annotation2))
                    ) {
                        return false;
                    }
                } catch (Exception e) {
                    throw new ReflectionsException(
                        String.format(
                            "could not invoke method %s on annotation %s",
                            method.getName(),
    ...
    private static boolean isAssignable(
        ...
        for (int i = 0; i < childClasses.length; i++) {
            if (!parentClasses[i].isAssignableFrom(childClasses[i])
                || (parentClasses[i] == Object.class
                && childClasses[i] != Object.class)
            ) {
                return false;
            }
        }
